...
class M_rw extends CI_Model {
	...
	var $table = 'tbl_wrw';
	var $column_order = array(null, 'tbl_wrw.nama_rw', 'tbl_wdesa.nama_desa', 'tbl_wkecamatan.nama_kec','tbl_wkecamatan.kode', null); //set column field database for datatable orderable
	var $column_search = array('tbl_wrw.nama_rw', 'tbl_wdesa.nama_desa','tbl_wkecamatan.nama_kec','tbl_wkecamatan.kode'); //set column field database for datatable searchable 
	var $order = array('tbl_wrw.id_rw' => 'desc'); // default order 
	...
	public function select_export() {
		$this->db->select('*');
		$this->db->from('tbl_wrw');
		$this->db->join('tbl_wkecamatan','tbl_wkecamatan.id_kec=tbl_wrw.id_kec', 'left');
		$this->db->join('tbl_wdesa','tbl_wdesa.id_desa=tbl_wrw.id_desa', 'left');
		$this->db->order_by('id_rw', 'desc');
	...
	public function get_list_rw()
	{
		...
		$this->db->order_by('tbl_wrw.id_rw','asc');
		$query = $this->db->get();
		$result = $query->result();
		...
		{
			$countries[$row->id_rw] = $row->nama_rw;
		}
		return $countries;
	}
	...
	{
		$this->db->select('tbl_wrw.id_rw, tbl_wrw.nama_rw, tbl_wdesa.nama_desa, tbl_wkecamatan.kode, tbl_wkecamatan.nama_kec');
		...
		{
			$this->db->like('tbl_wrw.nama_rw', $this->input->post('nama_rw'));
		}
		
		
		if($this->input->post('id_kec'))
		{
			$this->db->like('tbl_wrw.id_kec', $this->input->post('id_kec'));
		}
		
		if($this->input->post('id_desa'))
		{
			$this->db->like('tbl_wrw.id_desa', $this->input->post('id_desa'));
		}
	...
	public function select_by_wil() {
		...
		$sql1 = "SELECT DISTINCT nama_rw FROM tbl_wrw ";
