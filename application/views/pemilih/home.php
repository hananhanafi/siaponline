        ...
        <div class="panel panel-primary">
            <div class="panel-body">
                <div class="col-md-6 col-xs-12">
                    <?php 
                    echo '<h4 class="text-muted"><b>Calon Pemilih </b></h4>';
                    ?>
                </div>
                ...
                <div class="col-lg-2 col-xs-12">
                    <button  class="btn btn-default btn-block" onclick="delete_allpemilih()"><i class="fa fa-trash"></i> Hapus Data</button>
...
$(document).ready(function() {
    ...
    language: {
        ...
        "ajax": {
            "url": "<?php echo site_url('pemilih/ajax_list')?>",
            "type": "POST",
            "data": function ( data ) {
            }
        },
    ...
    $('#btn-export').click(function() {
        var startDate = $('#filter_from').val(); // Get start date from input
        var endDate = $('#filter_to').val();     // Get end date from input

        // Check if both start date and end date are selected
        if (startDate && endDate) {
            // Call the export function with the selected date range
            exportDataByDateRange(startDate, endDate);
        } else {
...
function exportDataByDateRange(startDate, endDate) {
    // Create a form element
    var form = document.createElement('form');
    form.setAttribute('method', 'post');
    form.setAttribute('action', '<?php echo base_url('pemilih/export'); ?>');
    form.setAttribute('target', '_blank');  // Open response in a new tab
    ...
    // Clean up and remove the form from the document
...
function add_person()
{
    ...
    // document.getElementById('nama_rt').addEventListener('change', perbaruiPilihanTPS);
...
function edit_pemilih(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('pemilih/ajax_edit')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            ...
            // perbaruiInputRT(data.kd_rt,data.kd_rw);
            // perbaruiInputTPS(data.kd_tps, data.kd_rt);
        },
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('pemilih/ajax_add')?>";
    } else {
        url = "<?php echo site_url('pemilih/ajax_update')?>";
    }
...
function delete_pemilih(id)
    {
        ...
        if (isConfirm) {
            ...
            $.ajax({
                url : "<?php echo site_url('pemilih/ajax_delete')?>/"+id,
                type: "POST",
                dataType: "JSON",
                error: function(jqXHR, textStatus, errorThrown) {
...
function delete_allpemilih()
    {
        ...
        if (isConfirm) {
            ...
            $.ajax({
                url : "<?php echo site_url('pemilih/ajax_all_delete')?>",
                type: "POST",
                dataType: "JSON",
                error: function(jqXHR, textStatus, errorThrown) {
                    alert('Error deleting data');
    ...
    function perbaruiPilihanTPS() {
        ...
        var rtTerpilih = dropdownRT.value;
        ...
        if (rtTerpilih !== '') {
            $.ajax({
                url : "<?php echo site_url('pemilih/ajax_get_tps')?>/" + rtTerpilih,
                type: "GET",
                dataType: "JSON",
                success: function(data)
                {
    ...
    function perbaruiInputTPS(id_tps_selected = null, id_rt_selected = null) {
        var dropdownRT = document.getElementById('nama_rt');
        var dropdownTPS = document.getElementById('nama_tps');
        var rtTerpilih = dropdownRT.value;
        if (id_rt_selected) {
            rtTerpilih = id_rt_selected;   
        }
