...
class M_rt extends CI_Model {
	...
	var $table = 'tbl_wrt';
	var $column_order = array(null, 'tbl_wrt.nama_rt', 'tbl_wrw.nama_rw', 'tbl_wdesa.nama_desa', 'tbl_wkecamatan.nama_kec','tbl_wkecamatan.kode', null); //set column field database for datatable orderable
	var $column_search = array('tbl_wrt.nama_rt', 'tbl_wrw.nama_rw', 'tbl_wdesa.nama_desa','tbl_wkecamatan.nama_kec','tbl_wkecamatan.kode'); //set column field database for datatable searchable 
	var $order = array('tbl_wrt.id_rt' => 'desc'); // default order 
	...
	public function select_export() {
		$this->db->select('*');
		$this->db->from('tbl_wrt');
		$this->db->join('tbl_wkecamatan','tbl_wkecamatan.id_kec=tbl_wrt.id_kec', 'left');
		
		$this->db->join('tbl_wdesa','tbl_wdesa.id_desa=tbl_wrt.id_desa', 'left');

		$this->db->join('tbl_wrw','tbl_wrw.id_rw=tbl_wrt.id_rw', 'left');
	...
	public function get_list_rt()
	{
		...
		if ($this->session->userdata('id_rw')) {
			$this->db->where("id_rw", $this->session->userdata('id_rw'));
		}
		$this->db->order_by('tbl_wrt.id_rt','asc');
		$query = $this->db->get();
		$result = $query->result();
		...
		{
			$countries[$row->id_rt] = $row->nama_rt;
		}
		return $countries;
	}
	...
	{
		$this->db->select('tbl_wrt.id_rt, tbl_wrt.nama_rt, tbl_wrw.nama_rw, tbl_wdesa.nama_desa, tbl_wkecamatan.kode, tbl_wkecamatan.nama_kec');
		...
		{
			$this->db->like('tbl_wrt.nama_rt', $this->input->post('nama_rt'));
		}
		
		
		if($this->input->post('id_kec'))
		{
			$this->db->like('tbl_wrt.id_kec', $this->input->post('id_kec'));
		}
		
		if($this->input->post('id_desa'))
		{
			$this->db->like('tbl_wrt.id_desa', $this->input->post('id_desa'));
		}
		
		if($this->input->post('id_rw'))
		{
			$this->db->like('tbl_wrt.id_rw', $this->input->post('id_rw'));
		}
	...
	public function get_datatables()
	{
		...
		// var_dump($this->db->print_out()); die();
		$query = $this->db->get();
	...
	public function select_by_wil() {
		...
		$sql1 = "SELECT DISTINCT nama_rt FROM tbl_wrt ";
  ...
  function getDesa($postData){
	...
	function getRW(){
		...
		$this->db->order_by('id_rw');
		$q = $this->db->get('tbl_wrw');
		$response = $q->result_array();
	...
	function getRT(){
		...
		// Select record
		$this->db->select('*');
		$this->db->order_by('nama_rt');
		$q = $this->db->get('tbl_wrt');
		$response = $q->result_array();
  ...
  public function select_jml_pemilih($kc) {
	...
	public function get_by_kelurahan($id)
	{
		$this->db->from($this->table);
